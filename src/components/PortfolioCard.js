...
function PortfolioCard({ project, setModalVisible, setFeatureApp }) {
	...
	return (
		<div className="portfolio-item" onMouseEnter={() => setMenuVisible(true)} onMouseLeave={() => setMenuVisible(false)}>
			{project.pinned ? (
			...
			<img className={menuVisible ? "blur" : ""} src={`./assets/${project.image}`} />
			<div className={`${menuVisible ? "visible" : "hidden"} portfolio-item-info`}>
				...
				<div className="portfolio-item-info-menu">
					{urlComponents.map((urlComponent, idx) => {
						if (urlComponent.url && urlComponent.url.length > 0) {
							return <UrlComponent key={idx} icon={urlComponent.icon} targetUrl={urlComponent.url} onClick={urlComponent.onClick} />;
						} else {
							return <></>;
						}
					})}
